...
class Visualizer:
    ...
    def draw_instance_predictions(self, predictions,vars = "pred_masks"):
        """
        ...
        if predictions.has(vars):
            if vars == "pred_edge":
                masks = np.asarray(predictions.pred_edge)
            elif vars == "pred_body":
                masks = np.asarray(predictions.pred_body)
            else:
        ...
        if self._instance_mode == ColorMode.IMAGE_BW:
            if vars == "pred_edge":
                self.output.img = self._create_grayscale_image(
                    (predictions.pred_edge.any(dim=0) > 0).numpy()
                )
            elif vars == "pred_body":
                self.output.img = self._create_grayscale_image(
                    (predictions.pred_body.any(dim=0) > 0).numpy()
                )
            else:
                self.output.img = self._create_grayscale_image(
