...
class FCOS(nn.Module):
    ...
    def forward(self, images, features, gt_instances=None, top_module_body=None,top_module_edge=None,top_module_final=None):
        """
        Arguments:
        ...
        logits_pred, reg_pred, ctrness_pred, top_feats, bbox_towers,cls_fea_fusion = self.fcos_head(
            features, top_module_body,top_module_edge,top_module_final, self.yield_proposal
        )
        ...
        if self.training:
            ...
            return results, losses,cls_fea_fusion
        else:
...
class FCOSHead(nn.Module):
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        self.cls_fusion_refine = nn.Conv2d(256*3,128,kernel_size=3,stride=1,padding=1)

    def forward(self, x, top_module_body=None,top_module_edge=None,top_module_final=None, yield_bbox_towers=False):
        logits = []
        bbox_reg = []
        ctrness = []
        top_feats= {'top_feats_body':[],'top_feats_edge':[],'top_feats_final':[]}
        bbox_towers = []

        for l, feature in enumerate(x):

            feature = self.share_tower(feature)
            # torch.Size([16, 256, 100, 168])
            # torch.Size([16, 256, 50, 84])
            # torch.Size([16, 256, 25, 42])
            # torch.Size([16, 256, 13, 21])
            # torch.Size([16, 256, 7, 11])
            cls_tower = self.cls_tower(feature)
            if l == 0:
                cls_fea_fusion = cls_tower
            else:
                if l<3:
                    target_h, target_w = cls_fea_fusion.size()[2:]
                    h, w = cls_tower.size()[2:]
                    assert target_h % h == 0
                    assert target_w % w == 0
                    factor_h, factor_w = target_h // h, target_w // w
                    assert factor_h == factor_w
                    x = aligned_bilinear(cls_tower, factor_h)
                    cls_fea_fusion = torch.cat((cls_fea_fusion,x),dim=1)
            ...
            if top_module_body is not None:
                top_feats['top_feats_body'].append(top_module_body(bbox_tower))
                top_feats['top_feats_edge'].append(top_module_edge(bbox_tower))
                top_feats['top_feats_final'].append(top_module_final(bbox_tower))
